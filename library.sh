...
fix_the_controller_extends_from_base_controller () {
  filename=$1
  [ -f "$filename" ] || continue
  filename_dir=$(dirname "${filename}")
  filename_base=$(basename "${filename}")
  ...
  echo -e "${YELLOW}Fixing the controller ${namespace_dir}/${filename_base}${NC}";
  sed -i "3 ause Illuminate\\\\Routing\\\\Controller;" $filename
  sed -i '3 a\\' $filename
  ex -e -s -c ":%s/extends \\\\BaseController/extends Controller/g" $filename -c wq
}
